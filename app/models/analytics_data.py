...
Stores predictive health analytics data and patient metrics generated by AI models, providing structured insights into patient health status over time.
...
class AnalyticsData(db.Document):
    ...
    metrics = db.DictField(
        required=True,
        help_text="Nested dictionary containing health metrics such as heart rate, blood pressure, glucose levels, etc."
    )

    # Results from AI predictive models assessing risk factors or health conditions
    prediction_results = db.DictField(
        required=True,
        help_text="Nested dictionary with predictive analytics results (e.g., diabetes risk, heart disease likelihood)."
    )
    ...
    meta = {
        'indexes': [
            {
                'fields': ['patient_id'],
                'name': 'patient_analytics_data_idx'
            },
            {
                'fields': ['generated_at'],
                'name': 'analytics_generated_at_idx'
            }
        ],
